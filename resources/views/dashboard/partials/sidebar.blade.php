    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 sticky top-20">
        ...
        <nav class="space-y-1">
            @if($user->isStudent())
                ...
                <a href="{{ route('student.cv', $locale) }}" class="flex items-center space-x-3 px-3 py-2.5 rounded-xl text-sm font-medium {{ request()->routeIs('student.cv*') ? 'bg-primary-50 text-primary-700' : 'text-slate-600 hover:bg-slate-50' }} transition">
